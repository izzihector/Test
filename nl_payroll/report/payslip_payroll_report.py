...
class PayrollReportMaster(models.AbstractModel):
    _name = 'report.nl_payroll.nl_payroll_report_payslip' 
    ...
    def generate_xlsx_report(self, workbook, data, payslips):
        ...
        band_order_content_sequence = workbook.add_format({'font_size':10, 'valign': 'vcenter', 'align': 'center', 'bold': False })
        band_order_content_amount = workbook.add_format({'font_size':10, 'valign': 'vcenter', 'align': 'right', 'bold': False, 'num_format': '"AFN" #,##0.00' })
        band_order_content = workbook.add_format({'font_size':10, 'valign': 'vcenter', 'bold': False })
        header_format1 = workbook.add_format({'font_size':12, 'bold': True, 'color':'black', 'bottom': True,'border': 0 })
        header_format2 = workbook.add_format({'font_size':11, 'valign': 'vcenter', 'bold': True, 'color':'black', 'bottom': True,'border': 0 })
        header_format3 = workbook.add_format({'font_size':10, 'valign': 'vcenter', 'bold': True, 'bg_color':'#B3DCE7', 'color':'black', 'bottom': True,'border': 2 })
        ...
        # Bank Data
        bank_data = {}
        office_name = ', '.join(name for name in set([pasylip.office_id.name for pasylip in payslips]))

        # # Fetch available salary rules:
        used_structures = []
        for sal_structure in payslips.struct_id:
        ...
        for used_struct in used_structures:
            ...
            col_no = 16
            # Fetch available salary rules:
            for item in payslips.struct_id.rule_ids:
            ...
            has_payslips = False
            counter = 1
            for slip in payslips.filtered(lambda c: c.state not in ['cancel']):
                current_net = 0

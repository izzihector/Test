MSetObjectiveNextYear = {
          ...
          handler(newVal, oldVal) {
            if (newVal > 5) {
              ...
              this.showFlashMessage(
                (message = "Max Five Objectives can be set."),
                (type = "error")
              );
            } else {
              lenObjectives = Object.keys(
                this.formData["set_next_year_objectives"]
              ).length;
              if (this.total_list_objectives_length > lenObjectives) {
                this.formData.set_next_year_objectives[
                  "objective_" + lenObjectives
                ] = { name: "", id: null, expected_outcome: "" };
              }
            }
          },
          immediate: false,
    ...
    methods: {
        ...
        writeDataToForm(data) {
            ...
            this.formData['setting_next_year_objectives'] = true
            if (data.value['next_year_objectives']) {
                lenObjectives = Object.keys(data.value['next_year_objectives']).length
                if (lenObjectives > 0) {
                    data.value['next_year_objectives'].forEach((obj, index) => {
                        this.formData.set_next_year_objectives['objective_'+ index] = {name: obj.name, id: obj.id , expected_outcome: obj.expected_outcome}
                    });
        ...
        async submitForm() {
            ...
            if (result) {
                ...
                if (this.appraisalData.value.metaInfo.setting_general_objective_state_next){
                    this.changeComponent('MSetLearningObjectiveNextYear')
                }
            }
        ...
        To be completed in discussion and agreement with the employee/supervisor/line manager. Objectives should be derived from the employee’s JD, the unit workplan and objectives, SCA strategic objectives and OD processes, challenges and gaps identified in the periovus review period. You may revise these objectives as needed durig the review period</p></legend>      
            <div class="py-1">
                <form class="form-group" @submit.prevent="submitForm">
                    ...
                    <fieldset>
                        <template v-for="(objective, key, index) in formData.set_next_year_objectives" :key="index">
                        <div class="grand-parent">
                          <div class="objectives-parent">
                            <div class="objectives-child">
                              <span style="width:100%;">Next Year Objective</span>
                              
                              <textarea class="form-control disabled-textarea next-year" style="height:140px;" rows="3" placeholder="Describe the objective in details here .." :id="['objective_'+objective.id]"  :disabled="readOnlyMode" 
                                  v-model="formData.set_next_year_objectives['objective_'+index].name"></textarea>
                            </div>
                          </div>
                          <div class="objectives-parent">
                            <div class="objectives-child">
                              <span style="width:100%;">Expected Outcome</span>
                              <textarea class="form-control disabled-textarea" style="height:140px;" rows="3" placeholder="Expected outcome..." :id="['expected_outcome_'+objective.id]" :disabled="readOnlyMode" 
                                  v-model="formData.set_next_year_objectives['objective_'+index].expected_outcome"></textarea>
                            </div>
                          </div>
                      </div>
                        ...
                        <a href="#" style="font-size: 14px;background: #875a7b;color: white;padding: 4px 12px;float: right;" @click.prevent="total_list_objectives_length++" v-if="!readOnlyMode">Add Line</a>
                    </fieldset>
                </form>
            </div>
        </div>

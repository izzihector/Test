SubmitForm = {
  ...
  data() {
    return {
      ...
      button2Text: "Sign and Submit To 2nd Line Supervisor",
      confirmationRequired: true,
      note: "Stage will change",
      currentState: "",
      confirmationNote: "",
    };
  },
  ...
  methods: {
    writeDataToForm(data) {
      ...
      this.formData["old_state"] = data.value["state"];

      if (this.submitType == "byManager") {
        this.nextStage = data.value.metaInfo.next_stage;
      } else {
        this.nextStage = data.value.metaInfo.next_stage;
      }
      this.nextStageDispaly =
        this.appraisalData.value.all_states[this.nextStage];
      this.formData["target_state"] = this.nextStage;
      this.currentState = this.appraisalData.value.metaInfo.state;
      if (this.currentState == "objective_setting") {
        this.confirmationRequired = false;
        this.note =
          "By validating and submitting these objectives, the appraisal will move to the 'Performance Period' stage - and the employee shall have access to review these objectives from his portal. Furthermore, you shall still be able to make necessary changes during the performance period if required.";
      } else if (this.currentState == "supervisor_review") {
        this.confirmationRequired = false;
        this.note =
          "by choosing to click on the 'Sign' buttons above, you hereby confirm that you have provided your feedback and comments with regards to this appraisal to the best of your knowledge. This confirmation, along with the date and time of your confirmation, will be logged by the system as your electronic signature for this document.";
        this.buttonText = "Sign and Finalize";
      } else if (this.currentState == "self_review") {
        this.confirmationRequired = true;
        this.confirmationNote =
          "I have provided answers to the self-assessment section of this appraisal to the best of my knowledge, and hereby submit the form to my supervisor for further processing.";
        this.note =
          "Please carefully review all sections, and make sure you have provided all necessary information. Please note that once submitted, you will no longer be able to modify your answers. Once your supervisor conducts your assessment, you will be notified through email and should be able to review them prior to concluding this appraisal. ";
      }
    },
    async submitForm(buttonClicked = 1) {
      ...
      if (this.confirmationRequired) {
        ...
        if (!this.checkedConfirmation) {
          this.confirmationErrorMessage = "You need to check the confirmation.";
          this.showFlashMessage(
            (message = this.confirmationErrorMessage),
            (type = "error")
          );
          return;
        }
      ...
      if (buttonClicked == 1) {
        ...
        if (this.currentState == "supervisor_review") {
          this.formData["target_state"] = "final_comments";
        }
      } else {
        this.button2Text = "Submiting...";
        if (this.currentState == "supervisor_review") {
          this.formData["target_state"] = this.nextStage;
        }
      }
      ...
      if (buttonClicked == 1) {
        ...
        if (this.currentState == "supervisor_review") {
          this.button2Text = "Sign and Submit To 2nd Line Supervisor";
        }
      }
    },
  },

PIPSubmitForm = {
    inject: ['showFlashMessage', 'closeFlashMessage', 'appraisalData', 'submitFunction'],
    data() {
        return {
            nextStage: '',
            nextStageDispaly: '',
            ...
            formData: {},
            disableButton: false,
            buttonText: "Finalize and Submit",
            button2Text: "Add New Targets and Review Date",
    ...
    methods: {
        writeDataToForm(data) {
            ...
            this.currentState = this.appraisalData.value.state
            this.byManager = this.appraisalData.value.metaInfo.byManager
            if (this.byManager) {
                if (this.currentState == 'planning') {
                    this.confirmationRequired = false
                    this.note = "By submtting, the employee will be notfied that their performance improvement plan period has started and that they can review their targets from their portal account. Once the review period is near completion, the system will automatically notify you to provide your review notes against the perofrmance of the employee during the defined period"
                } else if (this.currentState == 'assessment') {
                    this.confirmationRequired = false
                    this.note = "Validate and submit to final comments or add new review date"
                }
            } else {
                this.buttonText = 'Sign and submit'
                if (this.currentState == 'final_comments') {
                    this.confirmationRequired = false
                    this.note = "Sign and submit to the appraisal."
                }
            }

        },
        async submitForm(submitType = 'forward') {
            ...
            if (this.confirmationRequired) {
                ...
                if(!this.checkedConfirmation) {
                    ...
                    this.showFlashMessage(message=this.confirmationErrorMessage, type="error")
                    return
                }
            }
            if (submitType == 'forward') {
                this.buttonText = 'Submiting...'
            } else {
                this.formData['target_state'] = 'planning'
            ...
            if (submitType == 'forward') {
                this.buttonText = 'Finalize and Submit'
            } else {
                this.button2Text = 'Add New Targets and Review Date'
            }
        }
    },
    template: `
        <div class="text-center">
            ...
            <button class="btn btn-secondary btn-odoo bt-sm mt-3" @click="submitForm()" v-if="appraisalData.value.metaInfo.setSubmitAccess" :disabled="disableButton">{{ buttonText }}</button>
            <button class="btn btn-secondary btn-odoo bt-sm mt-3 ml-1" @click="submitForm('backward')" v-if="appraisalData.value.metaInfo.setSubmitAccess && appraisalData.value.state == 'assessment'" :disabled="disableButton">{{ button2Text }}</button>
            <div class="mt-3 text-muted" v-if="appraisalData.value.metaInfo.setSubmitAccess && appraisalData.value.state == 'assessment'">
                <small>
                    Clicking on ‘Finalize and Submit’ will end the performance imporvement plan process. Clicking on ‘Add New Targetrs and Review Dates’ 
                    will extend the performance improvement plan period. You will be redirected to the planning stage, where you can add new targets and set a new 
                    review date
                </small>
            </div>
        </div>

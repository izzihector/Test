SinglePipAppraisalParent = {
  props: ["id"],
  data() {
    return {
      currentComponent: "MSetPipObjectives",
      dataLoading: true,
      dataExists: false,
      appraisalData: {},
      error: false,
  ...
  methods: {
    ...
    async submitForm(data) {
        try {
            result = await  fetch('/supervisor/update-pip-employee-appraisals', {
                method: 'POST',
                headers: {
            ...
            if (jsonResponse.error) {
                throw new Error(`${jsonResponse.error.message} - ${jsonResponse.error.data.message}`);
            }
            // Custom Error Catch
    ...
    async getData() {
        try {
            result = await fetch('/supervisor/get-pip-employee-appraisals', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        if(Object.keys(this.appraisalData).length > 0) {
            ...
            if (this.appraisalData.state == 'planning'){
                this.change_component("MSetPipObjectives")
            } else if (this.appraisalData.state == 'assessment') {
                this.change_component("MSetPipReviews")
            } else {
    ...
    setAccessInfo() {
        ...
        this.hasAccessMessage = this.appraisalData.metaInfo.hasCurrentComponentAccessMessage
    },
  ...
  mounted() {
    console.log(">>>>> welcome id", this.id);
    this.getData();
  },
  template: `
        ...
        <div class="row my-5">
            ...
            <div class="col-lg-3 py-5 pl-4 bg-white bd-right-grey" v-if="!dataLoading && dataExists  && (hasAccess || currentComponent == 'PIPAppraisalStateReturner')">
                <side-bar @change_component="change_component" sideBarType="managers"/>
            </div>
            <div :class="{'col-lg-9': dataExists, 'col-lg-12': !dataExists || (!hasAccess && currentComponent != 'PIPAppraisalStateReturner')}" class="py-2 p-lg-5" style="background: white; min-height: 400px;">
                <template v-if="!dataLoading && dataExists">
                    <transition name="component-fade" mode="out-in">
                        <div class="alert custom_alert text-center d-block" v-if="!hasAccess && currentComponent != 'PIPAppraisalStateReturner'">
                            {{ hasAccessMessage }}
                        </div>  
                    </transition>
                    <transition name="component-fade" mode="out-in">
...
const single_pip_appraisal_app = Vue.createApp({});

single_pip_appraisal_app.component(
  "single-pip-appraisal-parent",
  SinglePipAppraisalParent
);
single_pip_appraisal_app.mount("#vue_pip_single_appraisal_app");
